...
int request::CheckForBody() {
  ...
  ss >> contentL_;
  (headers["content-length"].empty() && getMethod_() == "POST") ? throw 400 :
  (!headers["content-length"].empty() && contentL_ <= 0 && getMethod_() == "POST") ? throw 400 :
  (!headers["transfer-encoding"].empty() && headers["transfer-encoding"] != "chunked" && getMethod_() == "POST") ? throw 501 : contentL_ = 0;
  (headers["content-type"].empty() && getMethod_() == "POST") ? throw 400 : contentL_ = 0;
  return 1;
}
...
void request::parseheaders(std::string &page) {
  ...
  // if (pos == std::string::npos)
    // std::cout << "bad request" << std::endl; // internl server error !!!!
