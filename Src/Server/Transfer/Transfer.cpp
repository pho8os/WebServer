...
// void Transfer::parse2pages(std::string &page, Transfer &A)
// {
// 	static bool a;
// 	if(!a)
// 		A.parseheaders(page, A);
// 	a = true;
// 	if(A.page1.empty())
...
// 	if(!A.validboundary(A.page1 + A.page2, A))
// 	{
...
void parseboundary(std::string chunk, Transfer &A)
{
	...
	{	
		...
		if(A.fd > 0)
			close(A.fd);
		std::string filename = "/Users/mnassi/goinfre/up/" + file;
...
// void Transfer::parsepage(std::string &page, Transfer &A)
// {
// 	static bool a;
// 	if(!a)
// 		parseheaders(page, A);
